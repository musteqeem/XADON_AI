...
const replaceVars = (text, data) => {
    return text
       ...
       .replace(/@reason/g, data.reason || 'Left')
...
module.exports = {
    name: 'setgoodbye',
    alias: ['goodbye', 'setleave'],
    desc: 'Set goodbye message with 20 dynamic parameters',
    category: 'Admin',
    groupOnly: true,
    adminOnly: true,
    ...
    execute: async (sock, m, { args, text, reply }) => {
        ...
        if (!sub || sub === 'status') {
            const status = db[m.chat].goodbyeEnabled? 'ACTIVE' : 'INACTIVE';
            return reply(
`✦ ───── ⋆⋅☆⋅⋆ ───── ✦
...
╭─֎ *DEFENSE CORE*
│ ❏ Status : ${status}
│ ❏ Message : ${db[m.chat].goodbye? 'SET' : 'NOT SET'}
╰─────────────────────────╯
...
Goodbye messages are now DISABLED.`
        ...
        if (sub === 'clear') {
            db[m.chat].goodbye = null;
            db[m.chat].goodbyeEnabled = false;
            writeDB(db);
            return reply('֎ Goodbye message cleared and disabled.');
        }
...
╰─────────────────────────╯`
        ...
        if (sub === 'test') {
            ...
            if (!db[m.chat].goodbye) {
                return reply('֎ No goodbye message set. Use.setgoodbye <text> first.');
            }

            const preview = replaceVars(db[m.chat].goodbye, testData);
            return reply(
`✦ ───── ⋆⋅☆⋅⋆ ───── ✦
   ֎ • GOODBYE PREVIEW •
✦ ───── ⋆⋅☆⋅⋆ ───── ✦
...
Type.setgoodbye params to see all 20 parameters.`
            );
        }
...
Type.setgoodbye test to preview.`
        );
    }
};