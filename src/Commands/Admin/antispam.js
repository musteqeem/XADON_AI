...
│ ❏ Mode : antispam delete/warn/kick/mute
│ ❏ Settings : antispam max <n> / time <s> / duration <s>
│ ❏ Check : antispam warncount @user
│ ❏ List : antispam listwarns
│ ❏ Reset : antispam resetwarn @user
│ ❏ Nuke : antispam resetall
│ ❏ Unmute : antispam unmute @user
╰─────────────────────────╯`
            );
        }
        ...
        if (sub === 'max' && args[1]) {
            ...
            if (isNaN(num) || num < 2) return reply('_*❏ Min 2 messages required*_');
        ...
        if (sub === 'time' && args[1]) {
            ...
            if (isNaN(secs) || secs < 2) return reply('_*❏ Min 2 seconds required*_');
        ...
        if (sub === 'duration' && args[1]) {
            const secs = parseInt(args[1]);
            if (isNaN(secs) || secs < 5) return reply('_*❏ Min 5 seconds required*_');
        ...
        if (sub === 'resetwarn') {
            ...
            if (!target) return reply('_*✐ Usage*_ : ֎antispam resetwarn @user/reply/number');
        ...
        if (sub === 'warncount') {
            ...
            if (!target) return reply('_*✐ Usage*_ : ֎antispam warncount @user/reply/number');
        ...
        if (sub === 'unmute') {
            ...
            if (!target) return reply('_*✐ Usage*_ : ֎antispam unmute @user/reply/number');

            target = norm(target);
            const muteKey = `${group}:${target}`;
            const deleted = MUTE_CACHE.delete(muteKey);
            if (deleted) {
                return reply(`_*🔊 User Unmuted*_\n◉ User : @${target.split('@')[0]}`, { mentions: [target] });
            }
        ...
        if (sub === 'stats') {
            ...
            const muted = Array.from(MUTE_CACHE.keys()).filter(k => k.startsWith(group + ':')).length;
        ...
        reply('_*✐ Usage*_ : ֎antispam on/off/delete/warn/kick/mute/max/time/duration/warncount/listwarns/resetwarn/resetall/unmute/stats');
    }
};
        ...
        if (userData.count >= maxMessages) {
            ...
            if (action === 'delete') {
                ...
                setTimeout(() => {
                    ...
                    sock.sendMessage(group, { text: `_*🔊 User Unmuted*_\n◉ @${sender.split('@')[0]}`, mentions: [sender] }).catch(() => {});
                }, muteDuration);
            }
            else if (action === 'kick') {
                await sock.sendMessage(group, {
