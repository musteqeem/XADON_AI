const chatbot = require('../Core/❚.js');

module.exports = {
    name: 'planai',
    alias: [],
    category: 'AI',
    desc: 'Turn the request into a practical step-by-step plan with milestones, risks, and verification steps.',
    usage: '.planai <prompt>',

    execute: async (sock, m, { args = [], reply }) => {
        try {
            let input = Array.isArray(args) ? args.join(' ').trim() : '';
            if (!input) input = String(m?.quoted?.text || '').trim();

            if (!input) {
                return reply('❌ Provide a prompt or reply to a message.');
            }

            let userPrompt = input;

            const result = await chatbot.askAI('Turn the request into a practical step-by-step plan with milestones, risks, and verification steps.' + `\n\nUser input:\n${userPrompt}`, m.chat);

            return reply(result || '⚠️ The AI returned an empty response.');
        } catch (error) {
            console.error('[PLANAI ERROR]', error);
            return reply('⚠️ AI service is temporarily unavailable. Please try again.');
        }
    }
};
