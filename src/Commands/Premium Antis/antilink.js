...
│ ❏ Toggle : premantilink on/off
│ ❏ Mode : premantilink delete/warn/kick
│ ❏ Add Domain : premantilink add <domain>
│ ❏ Remove Domain : premantilink remove <domain>
│ ❏ Allow Link : premantilink allow <url>
│ ❏ Permit Prefix : premantilink permit <url>
│ ❏ Lists : premantilink allowlist/permitlist/domainlist
│ ❏ Check : premantilink warncount @user
│ ❏ List Warns : premantilink listwarns
│ ❏ Reset : premantilink resetwarn @user
│ ❏ Nuke : premantilink resetall
│ ❏ Clear : premantilink clear
╰─────────────────────────╯`
            );
        }
        ...
        if (sub === 'on') {
            ...
            return reply(`_*◉ Premantilink ACTIVE*_\n❏ Mode : ${actionText}`);
        }

        if (sub === 'off') {
            cfg.enabled = false;
            saveDB(db);
            return reply('_*◉ Premantilink INACTIVE*_');
        }
        ...
        if (sub === 'add') {
            ...
            if (!domain) return reply('_*✐ Usage*_ : ֎premantilink add <domain>\n֎ Example : github.com');
        ...
        if (sub === 'remove') {
            const domain = args[1]?.trim().toLowerCase();
            if (!domain) return reply('_*✐ Usage*_ : ֎premantilink remove <domain>');
        ...
        if (sub === 'allow') {
            ...
            if (!url || !url.startsWith('http')) {
                return reply('_*✐ Usage*_ : ֎premantilink allow <full_url>');
            }

            if (cfg.whitelist.includes(url)) {
                return reply('_*❏ Link Already Allowed*_');
            }
        ...
        if (sub === 'disallow') {
            ...
            if (!url) {
                return reply('_*✐ Usage*_ : ֎premantilink disallow <full_url>');
            }

            const idx = cfg.whitelist.indexOf(url);

            if (idx === -1) {
                return reply('_*❏ Link Not Found*_');
            }
        ...
        if (sub === 'permit') {
            ...
            if (!url || !url.startsWith('http')) {
                return reply('_*✐ Usage*_ : ֎premantilink permit <url_prefix>');
            }

            if (cfg.permit.includes(url)) {
                return reply('_*❏ Prefix Already Permitted*_');
            }
        ...
        if (sub === 'unpermit') {
            ...
            if (!url) {
                return reply('_*✐ Usage*_ : ֎premantilink unpermit <url_prefix>');
            }

            const idx = cfg.permit.findIndex(p => p === url);

            if (idx === -1) {
                return reply('_*❏ Prefix Not Found*_');
            }
        ...
        if (sub === 'allowlist') {
            if (!cfg.whitelist.length) {
                return reply('_*❏ No Allowed Links*_');
            }
        ...
        if (sub === 'permitlist') {
            if (!cfg.permit.length) {
                return reply('_*❏ No Permitted Prefixes*_');
            }
        ...
        if (sub === 'domainlist') {
            if (!cfg.domains.length) {
                return reply('_*❏ No Whitelisted Domains*_');
            }
        ...
        if (sub === 'warncount') {
            ...
            if (!target) {
                return reply('_*✐ Usage*_ : ֎premantilink warncount @user/reply/number');
            }
        ...
        if (sub === 'resetwarn') {
            ...
            if (!target) {
                return reply('_*✐ Usage*_ : ֎premantilink resetwarn @user/reply/number');
            }
...
╰─────────────────────────╯`
        ...
        reply('_*✐ Usage*_ : ֎premantilink on/off/delete/warn/kick/add/remove/allow/disallow/permit/unpermit/lists/warncount/listwarns/resetwarn/resetall/stats/clear');
    }
};
...
module.exports.handleAntiLink = async function(sock, m) {
    try {
        ...
        console.log(
            `[XADON AI ANTILINK] ${action} → ${sender.split('@')[0]} | link detected`
        );

    } catch (err) {
        console.error(
            '[XADON AI ANTILINK ERROR]',
            err.message
        );
    }
};