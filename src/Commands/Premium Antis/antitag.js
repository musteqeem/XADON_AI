...
function getMentions(m) {
    ...
    if (m.mentionedJid?.length) {
        mentions.push(...m.mentionedJid);
    }

    if (m.msg?.contextInfo?.mentionedJid?.length) {
        mentions.push(...m.msg.contextInfo.mentionedJid);
    }
...
│ ❏ Toggle : premantitag on/off
│ ❏ Mode : premantitag delete/warn/kick
│ ❏ Setting : premantitag min <number>
│ ❏ Check : premantitag warncount @user
│ ❏ List : premantitag listwarns
│ ❏ Reset : premantitag resetwarn @user
│ ❏ Nuke : premantitag resetall
╰─────────────────────────╯`
            );
        }
...
❏ Min Tags : ${db[group].minTags}`
        ...
        if (sub === 'off') {
            ...
            return reply('_*◉ Premantitag INACTIVE*_');
        }
        ...
        if (sub === 'min' && args[1]) {
            ...
            if (isNaN(num) || num < 1) {
                return reply('_*❏ Min 1 mention required*_');
            }
        ...
        if (sub === 'resetwarn') {
            ...
            if (!target) {
                return reply('_*✐ Usage*_ : ֎premantitag resetwarn @user/reply/number');
            }
        ...
        if (sub === 'warncount') {
            ...
            if (!target) {
                return reply('_*✐ Usage*_ : ֎premantitag warncount @user/reply/number');
            }
...
╰─────────────────────────╯`
        ...
        reply(
            '_*✐ Usage*_ : ֎premantitag on/off/delete/warn/kick/min <n>/warncount/listwarns/resetwarn/resetall/stats'
        );
    }
};
...
module.exports.handleAntiTag = async function(sock, m) {
    try {
        ...
        if (hasAllMention) {
            triggerReason = '@all mention';
        }
        else if (hasNonJid) {
            triggerReason = `hidetag (${nonJidMentionCount} non-JID)`;
        }
        else if (isHideTag) {
            triggerReason = 'hidetag invisible chars';
        }
        else {
            triggerReason = `${mentionCount} tags (min: ${minTags})`;
        }
        ...
        console.log(
            `[XADON AI ANTITAG] ${action} → ${sender.split('@')[0]} | ${triggerReason}`
        );

    } catch (err) {
        console.error(
            '[XADON AI ANTITAG ERROR]',
            err.message
        );
    }
};