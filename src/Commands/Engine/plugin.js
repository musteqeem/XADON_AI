...
const installPlugin = async (url) => {
    try {
        ...
        const fileName = `xdn_${category}_${cmdName}_${Date.now()}.js`;
...
╰─────────────────────────╯`
        ...
        if (success.length > 0) {
            success.forEach(s => rows.push(['✅', `${s.name} (${s.category})`]));
        }
        if (failed.length > 0) {
            failed.forEach(f => {
                const shortUrl = f.url.split('/').pop()?.slice(0, 30) || f.url;
                rows.push(['❌', `${shortUrl}: ${f.error}`]);
            });
        }

        return sock.sendMessage(m.chat, {
            headerText: `## ◈ AI Plugin Manager`,
            contentText: '---',
            title: '◈ Installation Results',
            table: rows,
            footerText: '💡 Use.plugins to list all installed plugins'
        }, { quoted: m });
    }
};
