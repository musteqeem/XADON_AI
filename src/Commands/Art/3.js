...
module.exports = {
    ...
    usage: ".kiss [@user] or reply to an image",
    examples: [".kiss", ".kiss @2347043550282", "reply to image +.kiss"],
    ...
    execute: async (sock, m, { args, reply, prefix }) => {
        ...
        if (!targetBuffer) {
            return reply(
`✦ ───── ⋆⋅☆⋅⋆ ───── ✦
 ֎ *${BOT_NAME} KISS EFFECT*
✦ ───── ⋆⋅☆⋅⋆ ───── ✦
╭─֎ *USAGE*
│ ❏ Reply to an image with ${prefix}kiss
│ ❏ ${prefix}kiss @mention
│ ❏ ${prefix}kiss <number>
╰─────────────────────────╯
_Powered by ${BOT_NAME}_`
        ...
        try {
            ...
            await sock.sendMessage(m.chat, {
                image: result,
                mimetype: "image/png",
                caption: `💋 _KISSED by ${BOT_NAME}_`
            }, { quoted: m });

            await sock.sendMessage(m.chat, { react: { text: '✓', key: m.key } });

        } catch (err) {
            console.error(`[${BOT_NAME} KISS ERROR]`, err);
            await sock.sendMessage(m.chat, { react: { text: '✘', key: m.key } });
            return reply(`✘ Failed to apply kiss effect`);
        }
    }
};