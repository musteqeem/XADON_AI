...
const CONFIG = {
    repo: process.env.REPO_NAME || 'musteqeem/XADON_AI',
...
async function newUpdate(sock, m, { reply }) {
    ...
    try {
        ...
        const zipRes = await axios.get(zipUrl, { 
            ...
            headers: { 'User-Agent': `${BOT_NAME}-Update-Checker` }
        });
        ...
        // Find files that exist locally but not in repo
...
❏ Branch : ${CONFIG.branch}\n\n`;
            ...
            if (newFiles.length > 0) {
                ...
                if (newFiles.length > 15) response += `  •...and ${newFiles.length - 15} more\n`;
                response += '\n';
            }
            ...
            if (changedFiles.length > 0) {
                ...
                if (changedFiles.length > 15) response += `  •...and ${changedFiles.length - 15} more\n`;
                response += '\n';
            }

            if (deletedFiles.length > 0) {
                response += `❏ Removed Files : ${deletedFiles.length}\n`;
                for (const f of deletedFiles.slice(0, 10)) {
                    response += `  • ${f}\n`;
                }
                if (deletedFiles.length > 10) response += `  •...and ${deletedFiles.length - 10} more\n`;
                response += '\n';
            }

            response += `❏ Action : Run .update to apply changes`;
        }
