...
// RAPID API ROTATION - put your keys in.env
...
const COUNTER_FILE = path.join(__dirname, '../../rapid_counter.json');
...
async function getAudioScraper(url) {
    ...
    try {
        ...
        if (!downloadUrl) throw new Error('Missing download link');
    ...
    // Try RapidAPI backup
    try {
        return await getAudioRapidAPI(videoId);
    } catch (e) { errors.push('rapidapi: ' + e.message); }
...
module.exports = {
    name: 'music',
    alias: ['song', 'ytmp3', 'play'],
    desc: '🎵 Download YouTube audio - Sends as document',
    category: 'Music',
    usage: '.music <song title or URL>',
    examples: ['.music assurance by Davido', '.music https://youtu.be/xxxxx'],
...
`✦ ───── ⋆⋅☆⋅⋆ ───── ✦
    *֎ • ${BOT_NAME} MUSIC DOWNLOADER 🎵*
✦ ───── ⋆⋅☆⋅⋆ ───── ✦
╭─֎ *📖 HOW TO USE*
│ ❏ Command : ${prefix}music <song title or URL>
╰─────────────────────────╯
╭─֎ *📝 EXAMPLES*
│ ❏ ${prefix}music assurance by Davido
│ ❏ ${prefix}music https://youtu.be/xxxxx
╰─────────────────────────╯`
        );
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    throw new Error('All scrapers and RapidAPI failed.\nScraper: ' + err.message + '\nRapidAPI: ' + err2.message);
                }
            }
