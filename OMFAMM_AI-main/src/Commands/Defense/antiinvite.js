...
module.exports = {
    ...
    alias: ['antiinv'],
    category: 'Defense',
    desc: 'Block WhatsApp group invite links with moderation actions',
    usage: '.antiinvite on/off | .antiinvite delete/warn/kick',
    groupOnly: true,
    adminOnly: true,
...
` +
                    `Whitelisted domains: ${cfg.whitelist.length}

` +
                    `• .antiinvite on/off
` +
                    `• .antiinvite delete/warn/kick
` +
                    `• .antiinvite allow <domain>
` +
                    `• .antiinvite disallow <domain>
` +
                    `• .antiinvite warncount @user
` +
                    `• .antiinvite resetwarn @user
` +
                    `• .antiinvite stats`
                );
            }
            ...
            if (sub === 'on' || sub === 'off') {
                cfg.enabled = sub === 'on';
                saveJSON(DB, db);
                return reply(`🛡️ ANTI INVITE ${cfg.enabled ? 'enabled' : 'disabled'}.`);
            }
...
` +
            ...
            return reply('❌ Unknown option. Use .antiinvite for the command list.');
        } catch (error) {
            console.error('[ANTIINVITE ERROR]', error);
            return reply(`❌ Error: ${error.message}`);
        }
    }
};

module.exports.handleAntiinvite = async function handleAntiinvite(sock, m) {
    try {
        ...
        let detected = /(chat\.whatsapp\.com\/|chat\.whatsapp\.com\b|wa\.me\/)/i.test(messageText(m));
        ...
        await enforce(sock, m, cfg, 'Unapproved group invite link', WARNS);
        console.log(`[DEFENSE][ANTIINVITE] ${sender} -> ${cfg.action}`);
    } catch (error) {
        console.error('[ANTIINVITE HANDLER ERROR]', error.message);
    }
};
