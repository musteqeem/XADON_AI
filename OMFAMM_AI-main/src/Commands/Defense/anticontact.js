...
const DB = 'anticontact';
const WARNS = 'anticontact_warns';
...
module.exports = {
    ...
    alias: ['contactguard'],
    category: 'Defense',
    desc: 'Block unsolicited contact-card messages in groups',
    usage: '.anticontact on/off | .anticontact delete/warn/kick',
    groupOnly: true,
    adminOnly: true,
...
` +
                    `• .anticontact on/off
` +
                    `• .anticontact delete/warn/kick
` +
                    `• .anticontact warncount @user
` +
                    `• .anticontact resetwarn @user
` +
                    `• .anticontact stats`
                );
            }
            ...
            if (sub === 'on' || sub === 'off') {
                cfg.enabled = sub === 'on';
                saveJSON(DB, db);
                return reply(`🛡️ ANTI CONTACT ${cfg.enabled ? 'enabled' : 'disabled'}.`);
            }
...
` +
            ...
            return reply('❌ Unknown option. Use .anticontact for the command list.');
        } catch (error) {
            console.error('[ANTICONTACT ERROR]', error);
            return reply(`❌ Error: ${error.message}`);
        }
    }
};

module.exports.handleAnticontact = async function handleAnticontact(sock, m) {
    try {
        ...
        let detected = Boolean(m.message?.contactMessage || m.message?.contactsArrayMessage);
        ...
        await enforce(sock, m, cfg, 'Unapproved contact card', WARNS);
        console.log(`[DEFENSE][ANTICONTACT] ${sender} -> ${cfg.action}`);
    } catch (error) {
        console.error('[ANTICONTACT HANDLER ERROR]', error.message);
    }
};
